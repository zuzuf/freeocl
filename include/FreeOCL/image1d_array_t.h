...
*/
#ifndef __FREEOCL_OPENCL_C_PREINCLUDE_IMAGE1D_ARRAY_T_H__
#define __FREEOCL_OPENCL_C_PREINCLUDE_IMAGE1D_ARRAY_T_H__
...
inline int __address_mode(const image1d_array_t &image, const sampler_t sampler, const int coord)
{
	int __coord;
	switch(sampler & 0xFF)
	{
...
inline float4 read_imagef(const image1d_array_t &image, sampler_t sampler, const int2 &coord)
{
	const int __coord = __address_mode(image, sampler, coord.get<0>());
	const int layer = clamp(coord.get<1>(), 0, image.array_size - 1);

	const char * const ptr = (const char*)image.data
							 + __coord * image.element_size
							 + layer * image.row_pitch;
	size_t nb_chan;
...
inline float4 read_imagef(const image1d_array_t &image, sampler_t sampler, const float2 &coord)
{
	float2 rcoord = coord;
	if (sampler & CLK_NORMALIZED_COORDS_TRUE)
		rcoord *= float2::make(image.width, image.array_size);
	rcoord.get<1>() = floor(0.5f + rcoord.get<1>());
	if (!(sampler & CLK_FILTER_LINEAR))
		return read_imagef(image, sampler, convert_int2(rcoord));
	const int2 c0 = convert_int2(rcoord - float2::make(0.5f, 0.f));
	const float2 dp = rcoord - convert_float2(c0);
	const float4 v0 = read_imagef(image, sampler, c0);
	const float4 v1 = read_imagef(image, sampler, c0 + int2::make(1,0));
	return v0 + dp.get<0>() * (v1 - v0);
}
inline float4 read_imagef(const image1d_array_t &image, const int2 &coord)
{
	return read_imagef(image, CLK_NORMALIZED_COORDS_FALSE | CLK_FILTER_NEAREST | CLK_ADDRESS_NONE, coord);
}

inline int4 read_imagei(const image1d_array_t &image, sampler_t sampler, const int2 &coord)
{
	const int __coord = __address_mode(image, sampler, coord.get<0>());
	const int layer = clamp(coord.get<1>(), 0, image.array_size - 1);
...
inline int4 read_imagei(const image1d_array_t &image, sampler_t sampler, const float2 &coord)
{
	float2 rcoord = coord;
	if (sampler & CLK_NORMALIZED_COORDS_TRUE)
	{
...
inline int4 read_imagei(const image1d_array_t &image, const int2 &coord)
{
	return read_imagei(image, CLK_NORMALIZED_COORDS_FALSE | CLK_FILTER_NEAREST | CLK_ADDRESS_NONE, coord);
}

inline uint4 read_imageui(const image1d_array_t &image, sampler_t sampler, const int2 &coord)
{
	const int __coord = __address_mode(image, sampler, coord.get<0>());
	const int layer = clamp(coord.get<1>(), 0, image.array_size - 1);
...
inline uint4 read_imageui(const image1d_array_t &image, sampler_t sampler, const float2 &coord)
{
	float2 rcoord = coord;
	if (sampler & CLK_NORMALIZED_COORDS_TRUE)
	{
...
inline uint4 read_imageui(const image1d_array_t &image, const int2 &coord)
{
	return read_imageui(image, CLK_NORMALIZED_COORDS_FALSE | CLK_FILTER_NEAREST | CLK_ADDRESS_NONE, coord);
}

inline void write_imagef(image1d_array_t &image, const int2 &coord, const float4 &color)
{
	size_t nb_chan;
...
inline void write_imagei(image1d_array_t &image, const int2 &coord, const int4 &color)
{
	size_t nb_chan;
...
inline void write_imageui(image1d_array_t &image, const int2 &coord, const uint4 &color)
{
	size_t nb_chan;
...
inline int get_image_width(const image1d_array_t &image)	{	return image.width;	}
inline int get_image_array_size(const image1d_array_t &image)	{	return image.array_size;	}
inline int get_image_channel_data_type(const image1d_array_t &image)	{	return image.channel_data_type;	}
inline int get_image_channel_order(const image1d_array_t &image)	{	return image.channel_order;	}
