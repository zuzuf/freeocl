...
inline int2 __address_mode(const image2d_array_t &image, const sampler_t sampler, const int2 &coord)
{
	int2 __coord;
	switch(sampler & 0xFF)
	{
...
inline float4 read_imagef(const image2d_array_t &image, sampler_t sampler, const int4 &coord)
{
	const int2 __coord = __address_mode(image, sampler, int2::make(coord.get<0>(), coord.get<1>()));
	const int layer = clamp(coord.get<2>(), 0, image.array_size - 1);
...
inline float4 read_imagef(const image2d_array_t &image, sampler_t sampler, const float4 &coord)
{
	float4 rcoord = coord;
	if (sampler & CLK_NORMALIZED_COORDS_TRUE)
		rcoord *= float4::make(image.width, image.height, image.array_size, 0.0f);
	rcoord.get<2>() = floor(0.5f + rcoord.get<2>());
	if (!(sampler & CLK_FILTER_LINEAR))
		return read_imagef(image, sampler, convert_int4(rcoord));
	const int4 c0 = convert_int4(rcoord - float4::make(0.5f, 0.5f, 0.0f, 0.f));
	const float4 dp = rcoord - convert_float4(c0);
	float4 v0 = read_imagef(image, sampler, c0);
	float4 v1 = read_imagef(image, sampler, c0 + int4::make(1,0,0,0));
	v0 += dp.get<0>() * (v1 - v0);
	v1 = read_imagef(image, sampler, c0 + int4::make(0,1,0,0));
	float4 v2 = read_imagef(image, sampler, c0 + int4::make(1,1,0,0));
	v1 += dp.get<0>() * (v2 - v1);
	return v0 + dp.get<1>() * (v1 - v0);
}
inline float4 read_imagef(const image2d_array_t &image, const int4 &coord)
{
	return read_imagef(image, CLK_NORMALIZED_COORDS_FALSE | CLK_FILTER_NEAREST | CLK_ADDRESS_NONE, coord);
}

inline int4 read_imagei(const image2d_array_t &image, sampler_t sampler, const int4 &coord)
{
	const int2 __coord = __address_mode(image, sampler, int2::make(coord.get<0>(), coord.get<1>()));
	const int layer = clamp(coord.get<2>(), 0, image.array_size - 1);
...
inline int4 read_imagei(const image2d_array_t &image, sampler_t sampler, const float4 &coord)
{
	float4 rcoord = coord;
	if (sampler & CLK_NORMALIZED_COORDS_TRUE)
	{
		rcoord *= float4::make(image.width, image.height, image.array_size, 0);
		rcoord.get<2>() = floor(0.5f + rcoord.get<2>());
...
inline int4 read_imagei(const image2d_array_t &image, const int4 &coord)
{
	return read_imagei(image, CLK_NORMALIZED_COORDS_FALSE | CLK_FILTER_NEAREST | CLK_ADDRESS_NONE, coord);
}

inline uint4 read_imageui(const image2d_array_t &image, sampler_t sampler, const int4 &coord)
{
	const int2 __coord = __address_mode(image, sampler, int2::make(coord.get<0>(), coord.get<1>()));
	const int layer = clamp(coord.get<2>(), 0, image.array_size - 1);
...
inline uint4 read_imageui(const image2d_array_t &image, sampler_t sampler, const float4 &coord)
{
	float4 rcoord = coord;
	if (sampler & CLK_NORMALIZED_COORDS_TRUE)
	{
...
inline uint4 read_imageui(const image2d_array_t &image, const int4 &coord)
{
	return read_imageui(image, CLK_NORMALIZED_COORDS_FALSE | CLK_FILTER_NEAREST | CLK_ADDRESS_NONE, coord);
}

inline void write_imagef(image2d_array_t &image, const int4 &coord, const float4 &color)
{
	size_t nb_chan;
	const float4 v = __map_channels_for_writing(image, color, nb_chan);
	char *const ptr = (char*)image.data
...
inline void write_imagei(image2d_array_t &image, const int4 &coord, const int4 &color)
{
	size_t nb_chan;
...
inline void write_imageui(image2d_array_t &image, const int4 &coord, const uint4 &color)
{
	size_t nb_chan;
...
inline int get_image_width(const image2d_array_t &image)	{	return image.width;	}
inline int get_image_height(const image2d_array_t &image)	{	return image.height;	}
inline int get_image_array_size(const image2d_array_t &image)	{	return image.array_size;	}
inline int get_image_channel_data_type(const image2d_array_t &image)	{	return image.channel_data_type;	}
inline int get_image_channel_order(const image2d_array_t &image)	{	return image.channel_order;	}

inline int2 get_image_dim(const image2d_array_t &image)
{
