...
inline int4 __address_mode(const image3d_t &image, const sampler_t sampler, const int4 &coord)
{
	int4 __coord;
	switch(sampler & 0xFF)
	{
	...
	case CLK_ADDRESS_CLAMP_TO_EDGE:
		__coord = clamp(coord, int4::make(0), int4::make(image.width - 1, image.height - 1, image.depth - 1, 0));
		break;
	case CLK_ADDRESS_CLAMP:
		__coord = clamp(coord, int4::make(-1), int4::make(image.width, image.height, image.depth, 0));
		break;
	case CLK_ADDRESS_REPEAT:
		__coord = coord % int4::make(image.width, image.height, image.depth, 1);
		break;
	case CLK_ADDRESS_MIRRORED_REPEAT:
		__coord = coord % int4::make(image.width << 1, image.height << 1, image.depth << 1, 1);
		__coord = abs(__coord - int4::make(image.width, image.height, image.depth, 0));
		break;
	}
	return __coord;
}
...
{
	const int4 __coord = __address_mode(image, sampler, coord);
...
inline float4 read_imagef(const image3d_t &image, sampler_t sampler, const float4 &coord)
{
	...
	if (sampler & CLK_NORMALIZED_COORDS_TRUE)
		rcoord *= float4::make(image.width, image.height, image.depth, 0.0f);
	if (!(sampler & CLK_FILTER_LINEAR))
		return read_imagef(image, sampler, convert_int4(rcoord));
	const int4 c0 = convert_int4(rcoord - float4::make(0.5f, 0.5f, 0.5f, 0.f));
	...
	v1 = read_imagef(image, sampler, c0 + int4::make(0,0,1,0));
	v2 = read_imagef(image, sampler, c0 + int4::make(1,0,1,0));
	v1 += dp.get<0>() * (v2 - v1);
	v2 = read_imagef(image, sampler, c0 + int4::make(0,1,1,0));
	const float4 v3 = read_imagef(image, sampler, c0 + int4::make(1,1,1,0));
	v2 += dp.get<0>() * (v3 - v2);
	v1 += dp.get<1>() * (v2 - v1);

	return v0 + dp.get<2>() * (v1 - v0);
}
inline float4 read_imagef(const image3d_t &image, const int4 &coord)
{
...
{
	if (sampler & CLK_NORMALIZED_COORDS_TRUE)
		return read_imagei(image, sampler, convert_int4(coord * float4::make(image.width, image.height, image.depth, 0)));
	return read_imagei(image, sampler, convert_int4(coord));
}
inline int4 read_imagei(const image3d_t &image, const int4 &coord)
{
...
{
	if (sampler & CLK_NORMALIZED_COORDS_TRUE)
		return read_imageui(image, sampler, convert_int4(coord * float4::make(image.width, image.height, image.depth, 0)));
	return read_imageui(image, sampler, convert_int4(coord));
}
inline uint4 read_imageui(const image3d_t &image, const int4 &coord)
{
